...
--查询有提成的员工信息（last name,  job, salary, and commission），并按工资降序排列。
...
--Find the job that was filled in the first half of 1990 and the same job that was filled during the same period in 1991.
select job_id
from employees
...
--Write a query to display the last names, month of the date of join, and hire date of those employees who have joined in the month of January, irrespective of the year of join.
...
--Write a query to delete the oldest JOB_HISTORY row of  an employee by looking up the JOB_HISTORY table for the MIN(START_DATE) for the employee. Delete the records of only those employees who have changed at least two jobs. If your query executes correctly, you will get the feedback:
...
--Write a query to display the job IDs of those jobs whose maximum salary is above half the maximum salary in the whole company. Use the WITH clause to write this query. Name the query MAX_SAL_CALC.
with
MAX_SAL_CALC as (
...
--Write a hierarchical query to display the employee number, manager number, and employee last name for all employees who are two levels below employee De Haan (employee number 102). Also display the level of the employee.
...
--Query the SPECIAL_SAL, SAL_HISTORY and the MGR_HISTORY tables to view the inserted records.
select *
from special_sal
select *
from sal_history
...
--Create the LOCATIONS_NAMED_INDEX table based on the following table instance chart. Name the index for the PRIMARY KEY column as LOCATIONS_PK_IDX.
...
--Query the USER_INDEXES table to display the INDEX_NAME for the LOCATIONS_NAMED_INDEX table
select index_name
from user_indexes
where table_name = 'LOCATIONS_NAMED_INDEX'
```